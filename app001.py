...
def modelo_popularidade_ev(historico: np.ndarray) -> np.ndarray:
    ...
    popularidade[numeros % 6 == 0] *= 1.05  # fix: índice 60 era out-of-bounds; usar booleano
...
def executar_backtest(
    ...
    n_testes: int,
    passo: int,
    _progress_callback=None,  # opcional: fn(float 0-1) para barra de progresso
) -> ResultadoBacktest:
    ...
    for _i, idx in enumerate(indices):
        if _progress_callback is not None:
            _progress_callback((_i + 1) / len(indices))
...
if executar or "analise" not in st.session_state or config_mudou:
    _barra = st.progress(0, text="Backtesting: aguarde...")
    historico_tuple = tuple(tuple(int(v) for v in row) for row in historico)
    bt = executar_backtest(
        historico_tuple, janela_treino, n_testes, passo,
        _progress_callback=lambda v: _barra.progress(v, text=f"Backtesting: {v:.0%} concluído"),
    )
    _barra.empty()
...
with abas[1]:
    ...
    # Paleta qualitativa com 10 cores distintas (Plotly Safe)
    _PALETA = [
        "#636EFA", "#EF553B", "#00CC96", "#AB63FA", "#FFA15A",
        "#19D3F3", "#FF6692", "#B6E880", "#FF97FF", "#FECB52",
    ]

    # Identificar o melhor modelo (maior ganho acumulado final)
    _ganhos_finais = {
        nome: float(np.cumsum(ll_unif - np.array(bt.logloss_series[nome]))[-1])
        for nome in MODEL_FACTORIES if nome != "Uniforme"
    }
    _melhor = max(_ganhos_finais, key=_ganhos_finais.get)

    fig_cum = go.Figure()
    for _ci, nome in enumerate(n for n in MODEL_FACTORIES if n != "Uniforme"):
        ll = np.array(bt.logloss_series[nome])
        ganho_cum = np.cumsum(ll_unif - ll)
        _cor = _PALETA[_ci % len(_PALETA)]
        _destaque = nome == _melhor
        fig_cum.add_trace(go.Scatter(
            y=ganho_cum, mode="lines", name=nome,
            opacity=1.0 if _destaque else 0.45,
            line=dict(width=3 if _destaque else 1.2, color=_cor,
                      dash="solid" if _destaque else "dot"),
        ))
    fig_cum.add_hline(y=0, line_dash="dash", line_color="white", line_width=1)
    fig_cum.add_annotation(
        text=f"↑ Melhor: {_melhor}", xref="paper", yref="paper",
        x=0.01, y=0.97, showarrow=False,
        font=dict(size=11, color="#B6E880"), align="left",
    )
...
with abas[2]:
    ...
    p_prev_all, y_real_all = [], []
    for p_ens, real in zip(bt.previsoes_ensemble, bt.resultados_reais):
        p_marg = np.clip(p_ens * DEZENAS_POR_JOGO, 0, 1)
        y = np.zeros(N_NUMEROS); y[real - 1] = 1
        p_prev_all.extend(p_marg.tolist())
        y_real_all.extend(y.tolist())

    p_prev_all = np.array(p_prev_all)
    y_real_all = np.array(y_real_all)
    bins = np.unique(np.quantile(p_prev_all, np.linspace(0, 1, n_bins + 1)))

    freq_obs, freq_prev, contagens = [], [], []
    for i in range(len(bins) - 1):
        mask = (p_prev_all >= bins[i]) & (p_prev_all < bins[i + 1])
        if mask.sum() > 0:
            freq_obs.append(float(y_real_all[mask].mean()))
            freq_prev.append(float(p_prev_all[mask].mean()))
            contagens.append(int(mask.sum()))

    freq_obs = np.array(freq_obs)
    freq_prev = np.array(freq_prev)
    contagens = np.array(contagens)
    fig_cal = go.Figure()
    fig_cal.add_trace(go.Scatter(
        x=[0, 1], y=[0, 1], mode="lines",
        line=dict(dash="dash", color="white", width=1), name="Calibração perfeita",
    ))
    fig_cal.add_trace(go.Scatter(
        x=freq_prev, y=freq_obs, mode="lines+markers",
        marker=dict(size=(8 + contagens / max(contagens.max(), 1) * 12).astype(int), color="#667eea"),
        error_y=dict(
            type="data",
            array=np.sqrt(freq_obs * (1 - freq_obs) / (contagens + 1)),
            visible=True, color="#a78bfa",
        ),
        name="Ensemble (p_marginal)", line=dict(color="#667eea", width=2),
    ))
    fig_cal.update_layout(
        xaxis_title="Probabilidade prevista (marginal)", yaxis_title="Frequência observada",
        template="plotly_dark", height=420,
        xaxis=dict(range=[0, max(0.15, float(p_prev_all.max()) * 1.15)]),
        yaxis=dict(range=[0, max(0.15, float(y_real_all.mean()) * 2.0)]),
    )
    st.plotly_chart(fig_cal, use_container_width=True)

    ece = float(np.average(np.abs(freq_obs - freq_prev), weights=contagens))
    st.metric("ECE — Expected Calibration Error", f"{ece:.5f}",
              help="Quanto mais próximo de zero, melhor. Não existe um patamar universal; interprete junto com o tamanho da amostra e os bins.")
    st.info(
        "Pontos acima da diagonal: modelo subestima a probabilidade real. "
        "Abaixo: superestima. O tamanho dos círculos é proporcional ao número de amostras no bin."
    )
